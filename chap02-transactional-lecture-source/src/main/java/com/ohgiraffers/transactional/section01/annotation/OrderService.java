...
public class OrderService {
    ...
    /* 설명. 서비스 메소드 내부에서 모든 DML 관련 작업에 예외없이 잘 동작하면 commit 예외 발생 시, Rollback 된다.
      * [전파행위 옵션]
      ...
      *                동시성 문제는 모두 해소되지만 성능은 현저하게 떨어진다.(모든 동시성 문제는 막을 수 없다.)
...
//        System.out.println("totalOrderPrice = " + totalOrderPrice);
        ...
        /*  insert를 하기 위해 테이블과 매칭되는 Entity로 옮겨 담는다.(DTO -> Entity)*/
...
//        System.out.println("oMenus = " + oMenus);
        ...
        for (int i = 0; i < orderMenusSize; i++) {
        ...
        System.out.println("orderMenusSize = " + orderMenusSize);

    }

    /* 설명. 주문건에 대한 총 합계 금액 계산 메소드 */
