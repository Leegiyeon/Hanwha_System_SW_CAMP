...
public class ResponseTestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
         ...
         *  3. 응답하기(문자열을 통한 동적인 웹 페이지를 만들어서 스트림을 통해 내보내기)(SSR 기술)
        * */
        ...
        // memo. tomcat 10버전 이전(MIME타입과 인코딩 방식을 response 객체를 통해 따로 설정)
//        resp.setContentType("text/html");
//        resp.setCharacterEncoding("UTF-8");

        // memo. tomcat 10버전 이전(MIME타입과 인코딩 방식을 setContentType으로 한번에 설정)
        resp.setContentType("text/html; charset=UTF-8");

        // memo. tomcat 10버전 이후(MIME타입만 명시해도 인코딩 적용됨)
//        resp.setContentType("text/html");
