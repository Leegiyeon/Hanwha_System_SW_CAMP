...
//            menu.setMenuPrice(rset.getInt("MENU_PRICE"));
     ...
     *   - 상속 문제: 객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브 타입 엔터티이다.(서브타입을 별도 클래스로 나누었을 때)
     *               슈퍼 타입의 모든 속성을 서브타입이 공유하지 못하여 물리적으로 다른 테이블로 분리가 된 형태이다.
     *               (설계에 따라 서 하나의 테이블로 속성이 추가되기도 함)
     *               하지만 객체지향의 상속은 슈퍼타입의 속성을 공유해서 사용하므로 여기에서 패러다임의 불일치가 발생한다.
     *   - 연관관계, 겍체 그래프 탐색 문제: 객체지향에서 말하는 연관 관계(Association, Collection)는 데이터 베이스 저장 구조와 다른 형태이다.
     *
     * 설명.
     *  - 데이터 베이스 테이블에 맞춘 객체 모델
     *  public class Menu {
    ...
    /* 설명. 4. 동일성 보장 문제 */
    @DisplayName("조회한 두개의 행을 담은 객체 동일성 비교")
    @Test
